...
public class ShulkerAura extends Module
{
    ...
    private EntityPlayer getTarget() {
        ...
        for (final EntityPlayer player : ShulkerAura.mc.world.playerEntities) {
            if (player != null && player != ShulkerAura.mc.player && player.getHealth() > 0.0f && ShulkerAura.mc.player.getDistance((Entity)player) < 5.0f && !OyVey.friendManager.isFriend(player.getName())) {
                temp = player;
            }
        }
            ...
            if (this.detonate) {
                if (this.waitTicks++ > this.detonateDelay.getValue()) {
                    ...
                    for (final Entity e : ShulkerAura.mc.world.loadedEntityList) {
                        if (e instanceof EntityEnderCrystal && e != null && !e.isDead && ShulkerAura.mc.player.getDistance(e) < 5.0f) {
    ...
    public int findHotbarItem(final Item itemIn) {
        for (int i = 0; i < 9; ++i) {
            ...
            final Item item;
            if (stack != ItemStack.EMPTY && stack.getItem() instanceof Item && (item = stack.getItem()) == itemIn) {
                return i;
            }
        }
        return -1;
