...
public class Surround2 extends Module
{
    ...
    public Surround2() {
        super("Surround2", "Surrounds you with Obsidian", Category.COMBAT, true, false, false);
    ...
    private void doFeetPlace() {
        ...
        if (!EntityUtil.isSafe((Entity)Surround.mc.player, 0.0, true)) {
            this.isSafe = 0;
            this.placeBlocks(Surround.mc.player.getPositionVector(), EntityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, 0.0, true), true, false, false);
        }
        else if (!EntityUtil.isSafe((Entity)Surround.mc.player, -1.0, false)) {
            this.isSafe = 1;
            this.placeBlocks(Surround.mc.player.getPositionVector(), EntityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, -1.0, false), false, false, true);
        }
        else {
            this.isSafe = 2;
        }
    ...
    private Vec3d areClose(final Vec3d[] vec3ds) {
        ...
        for (final Vec3d vec3d : vec3ds) {
            for (final Vec3d pos : EntityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, 0.0, true)) {
