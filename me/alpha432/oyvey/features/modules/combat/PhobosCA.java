...
public class PhobosCA extends Module
{
    ...
    public PhobosCA() {
        super("PhobosCA", "Best CA on the market", Category.COMBAT, true, false, false);
        ...
        PhobosCA.instance = this;
    }
    
    public static PhobosCA getInstance() {
        if (PhobosCA.instance == null) {
            PhobosCA.instance = new PhobosCA();
        }
        return PhobosCA.instance;
    }
    ...
    public void onToggle() {
        PhobosCA.brokenPos.clear();
        PhobosCA.placedPos.clear();
    ...
    public String getDisplayInfo() {
        ...
        if (PhobosCA.target != null) {
            return PhobosCA.target.getName();
        }
        return null;
    }
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)PhobosCA.mc.world) instanceof EntityEnderCrystal) {
            pos = packet3.getEntityFromWorld((World)PhobosCA.mc.world).getPosition();
            if (this.removeAfterAttack.getValue()) {
                Objects.requireNonNull(packet3.getEntityFromWorld((World)PhobosCA.mc.world)).setDead();
                PhobosCA.mc.world.removeEntityFromWorld(packet3.entityId);
            }
        }
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)PhobosCA.mc.world) instanceof EntityEnderCrystal) {
            final EntityEnderCrystal crystal = (EntityEnderCrystal)packet3.getEntityFromWorld((World)PhobosCA.mc.world);
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && PhobosCA.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : PhobosCA.mc.world.playerEntities) {
                        if (friend != null && !PhobosCA.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && OyVey.friendManager.isFriend(friend)) {
                ...
                if (PhobosCA.placedPos.contains(pos.down())) {
                    Label_0623: {
                        if (this.isRightThread() && this.superSafe.getValue()) {
                            ...
                            if ((selfDamage = DamageUtil.calculateDamage(pos, (Entity)PhobosCA.mc.player)) - 0.5 <= EntityUtil.getHealth((Entity)PhobosCA.mc.player)) {
                    ...
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(pos, (Entity)PhobosCA.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)PhobosCA.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (final EntityPlayer player : PhobosCA.mc.world.playerEntities) {
            ...
            for (final int id : packet4.getEntityIDs()) {
                final Entity entity = PhobosCA.mc.world.getEntityByID(id);
                if (entity instanceof EntityEnderCrystal) {
                    PhobosCA.brokenPos.remove(new BlockPos(entity.getPositionVector()).down());
                    PhobosCA.placedPos.remove(new BlockPos(entity.getPositionVector()).down());
                }
            }
        }
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity((World)PhobosCA.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer)packet5.getEntity((World)PhobosCA.mc.world), new Timer());
            }
        }
        else {
            ...
            if (event.getPacket() instanceof SPacketSoundEffect && (packet6 = (SPacketSoundEffect)event.getPacket()).getCategory() == SoundCategory.BLOCKS && packet6.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                ...
                if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && PhobosCA.mc.player != null && PhobosCA.mc.player.getDistanceSq(pos) < MathUtil.square(this.soundPlayer.getValue())) {
    ...
    private boolean predictSlowBreak(final BlockPos pos) {
        return this.antiCommit.getValue() && PhobosCA.lowDmgPos.remove(pos) && this.shouldSlowBreak(false);
    }
    
    private boolean isRightThread() {
        return PhobosCA.mc.isCallingFromMinecraftThread() || (!OyVey.eventManager.ticksOngoing() && !this.threadOngoing.get());
    }
    ...
    private void attackCrystalPredict(final int entityID, final BlockPos pos) {
        ...
        PhobosCA.mc.player.connection.sendPacket((Packet)attackPacket);
        if (this.breakSwing.getValue()) {
            PhobosCA.mc.player.connection.sendPacket((Packet)new CPacketAnimation(EnumHand.MAIN_HAND));
        }
    ...
    private void removePos(final BlockPos pos) {
        if (this.damageSync.getValue() == DamageSync.PLACE) {
            if (PhobosCA.placedPos.remove(pos)) {
                this.posConfirmed = true;
            }
        }
        else if (this.damageSync.getValue() == DamageSync.BREAK && PhobosCA.brokenPos.remove(pos)) {
            this.posConfirmed = true;
        }
    }
    ...
    private void postProcessBreak() {
        while (!this.packetUseEntities.isEmpty()) {
            ...
            PhobosCA.mc.player.connection.sendPacket((Packet)packet);
            if (this.breakSwing.getValue()) {
                PhobosCA.mc.player.swingArm(EnumHand.MAIN_HAND);
            }
            this.breakTimer.reset();
        }
    }
    ...
    private void handleWhile() {
        if (this.thread == null || this.thread.isInterrupted() || !this.thread.isAlive() || (this.syncroTimer.passedMs(this.syncThreads.getValue()) && this.syncThreadBool.getValue())) {
            if (this.thread == null) {
                this.thread = new Thread(RPhobosCA.getInstance(this));
            }
    ...
    private ScheduledExecutorService getExecutor() {
        ...
        service.scheduleAtFixedRate(RPhobosCA.getInstance(this), 0L, this.threadDelay.getValue(), TimeUnit.MILLISECONDS);
        return service;
    }
    
    public void doPhobosCA() {
    ...
    private boolean check() {
        ...
        this.mainHand = (PhobosCA.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL);
        this.offHand = (PhobosCA.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL);
        this.currentDamage = 0.0;
        this.placePos = null;
        if (this.lastSlot != PhobosCA.mc.player.inventory.currentItem || Surround.isPlacing) {
            this.lastSlot = PhobosCA.mc.player.inventory.currentItem;
        ...
        if ((!this.offHand && !this.mainHand && this.switchMode.getValue() == Switch.BREAKSLOT && !this.switching) || !DamageUtil.canBreakWeakness((EntityPlayer)PhobosCA.mc.player) || !this.switchTimer.passedMs(this.switchCooldown.getValue())) {
            this.renderPos = null;
            PhobosCA.target = null;
            return this.rotating = false;
        }
        if (this.mineSwitch.getValue() && Mouse.isButtonDown(0) && (this.switching || this.autoSwitch.getValue() == AutoSwitch.ALWAYS) && Mouse.isButtonDown(1) && PhobosCA.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) {
            this.switchItem();
        }
        this.mapCrystals();
    ...
    private void mapCrystals() {
        ...
        for (final Entity entity : PhobosCA.mc.world.loadedEntityList) {
            if (!entity.isDead && entity instanceof EntityEnderCrystal) {
                ...
                if (this.syncedFeetPlace.getValue() && entity.getPosition().down().equals((Object)this.syncedCrystalPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    float selfDamage = -1.0f;
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(entity, (Entity)PhobosCA.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)PhobosCA.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
        ...
        if (this.webAttack.getValue() && this.webPos != null) {
            if (PhobosCA.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue())) {
                this.webPos = null;
            }
            else {
                for (final Entity entity : PhobosCA.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) {
                        continue;
                    }
        ...
        if (this.shouldSlowBreak(true) && maxDamage < this.minDamage.getValue() && (PhobosCA.target == null || EntityUtil.getHealth((Entity)PhobosCA.target) > this.facePlace.getValue() || (!this.breakTimer.passedMs(this.facePlaceSpeed.getValue()) && this.slowFaceBreak.getValue() && Mouse.isButtonDown(0) && this.holdFacePlace.getValue() && this.holdFaceBreak.getValue()))) {
            this.efficientTarget = null;
            return;
        }
        if (this.packets.getValue() == 1) {
    ...
    private void placeCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue()) && this.place.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC || (this.switchMode.getValue() == Switch.BREAKSLOT && this.switching))) {
            ...
            if (PhobosCA.target != null && this.placePos != null) {
                if (!this.offHand && !this.mainHand && this.autoSwitch.getValue() != AutoSwitch.NONE && (this.currentDamage > this.minDamage.getValue() || (this.lethalSwitch.getValue() && EntityUtil.getHealth((Entity)PhobosCA.target) <= this.facePlace.getValue())) && !this.switchItem()) {
                    return;
                }
                if (this.currentDamage < this.minDamage.getValue() && this.limitFacePlace.getValue()) {
                    crystalLimit = 1;
                }
                if (this.currentDamage >= this.minMinDmg.getValue() && (this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || (this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals((Object)this.placePos))) && (this.currentDamage > this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtil.isArmorLow(PhobosCA.target, this.minArmor.getValue()) || EntityUtil.getHealth((Entity)PhobosCA.target) <= this.facePlace.getValue() || this.currentDamage > this.minDamage.getValue() || this.shouldHoldFacePlace())) {
                    final float damageOffset = (this.damageSync.getValue() == DamageSync.BREAK) ? (this.dropOff.getValue() - 5.0f) : 0.0f;
                    boolean syncflag = false;
                    if (this.syncedFeetPlace.getValue() && this.placePos.equals((Object)this.lastPos) && this.isEligableForFeetSync(PhobosCA.target, this.placePos) && !this.syncTimer.passedMs(this.damageSyncTime.getValue()) && PhobosCA.target.equals((Object)this.currentSyncTarget) && PhobosCA.target.getPosition().equals((Object)this.syncedPlayerPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    if (syncflag || this.currentDamage - damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            this.currentSyncTarget = PhobosCA.target;
                            this.syncedPlayerPos = PhobosCA.target.getPosition();
                            if (this.foundDoublePop) {
                                this.totemPops.put(PhobosCA.target, new Timer());
                            }
                            ...
                            if (this.addTolowDmg || (this.actualSlowBreak.getValue() && this.currentDamage < this.minDamage.getValue())) {
                                PhobosCA.lowDmgPos.add(this.placePos);
                            }
                            PhobosCA.placedPos.add(this.placePos);
    ...
    private boolean doSwitch() {
        if (!this.offhandSwitch.getValue()) {
            if (PhobosCA.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) {
                this.mainHand = false;
            }
            else {
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        final Block web;
        if (this.webAttack.getValue() && targettedPlayer != null && (web = PhobosCA.mc.world.getBlockState(playerPos = new BlockPos(targettedPlayer.getPositionVector())).getBlock()) == Blocks.WEB) {
            setToAir = playerPos;
            state = PhobosCA.mc.world.getBlockState(playerPos);
            PhobosCA.mc.world.setBlockToAir(playerPos);
        }
        for (final BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && PhobosCA.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue()), 1.0f)) {
                continue;
            }
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
            ...
            if (selfDamage + 0.5 >= EntityUtil.getHealth((Entity)PhobosCA.mc.player)) {
                continue;
            }
        ...
        if (setToAir != null) {
            PhobosCA.mc.world.setBlockState(setToAir, state);
            this.webPos = currentPos;
        }
        PhobosCA.target = currentTarget;
    ...
    private EntityPlayer getTarget(final boolean unsafe) {
        ...
        for (final EntityPlayer player : PhobosCA.mc.world.playerEntities) {
            if (!EntityUtil.isntValid((Entity)player, this.placeRange.getValue() + this.range.getValue()) && (!this.antiNaked.getValue() || !DamageUtil.isNaked(player))) {
                ...
                if (currentTarget == null) {
                    ...
                    if (PhobosCA.mc.player.getDistanceSq((Entity)player) >= PhobosCA.mc.player.getDistanceSq((Entity)currentTarget)) {
                        continue;
                    }
                    currentTarget = player;
                }
        ...
        if (this.predictPos.getValue() && currentTarget != null) {
            ...
            final EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP((World)PhobosCA.mc.world, profile);
    ...
    private void attackEntity(final Entity entity) {
        if (entity != null) {
            if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE && this.rotateFirst.getValue() && this.rotate.getValue() != Rotate.OFF) {
                ...
                PhobosCA.brokenPos.add(new BlockPos(entity.getPositionVector()).down());
            }
        }
    }
    ...
    private void manualBreaker() {
        if (this.rotate.getValue() != Rotate.OFF && this.eventMode.getValue() != 2 && this.rotating) {
            if (this.didRotation) {
                PhobosCA.mc.player.rotationPitch += (float)4.0E-4;
                this.didRotation = false;
            }
            else {
                PhobosCA.mc.player.rotationPitch -= (float)4.0E-4;
        ...
        if ((this.offHand || this.mainHand) && this.manual.getValue() && this.manualTimer.passedMs(this.manualBreak.getValue()) && Mouse.isButtonDown(1) && PhobosCA.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && PhobosCA.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && PhobosCA.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && PhobosCA.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = PhobosCA.mc.objectMouseOver) != null) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    final BlockPos mousePos = PhobosCA.mc.objectMouseOver.getBlockPos().up();
                    for (final Entity target : PhobosCA.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(mousePos))) {
    ...
    private void rotateTo(final Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(PhobosCA.mc.player.getPositionEyes(PhobosCA.mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private void rotateToPos(final BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                final float[] angle = MathUtil.calcAngle(PhobosCA.mc.player.getPositionEyes(PhobosCA.mc.getRenderPartialTicks()), new Vec3d((double)(pos.getX() + 0.5f), (double)(pos.getY() - 0.5f), (double)(pos.getZ() + 0.5f)));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    OyVey.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
    ...
    private boolean isValid(final Entity entity) {
        return entity != null && PhobosCA.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breakRange.getValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || PhobosCA.mc.player.canEntityBeSeen(entity) || (!PhobosCA.mc.player.canEntityBeSeen(entity) && PhobosCA.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breaktrace.getValue())));
    }
    ...
    static {
        PhobosCA.target = null;
        PhobosCA.lowDmgPos = (Set<BlockPos>)new ConcurrentSet();
        PhobosCA.placedPos = new HashSet<BlockPos>();
        PhobosCA.brokenPos = new HashSet<BlockPos>();
    }
    ...
    public static class PlaceInfo
    {
        ...
        public void runPlace() {
            BlockUtil.placeCrystalOnBlock(this.pos, this.offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND);
        }
    }
    
    private static class RPhobosCA implements Runnable
    {
        private static RPhobosCA instance;
        private PhobosCA PhobosCA;
        
        public static RPhobosCA getInstance(final PhobosCA PhobosCA) {
            if (RPhobosCA.instance == null) {
                RPhobosCA.instance = new RPhobosCA();
                RPhobosCA.instance.PhobosCA = PhobosCA;
            }
            return RPhobosCA.instance;
        }
        
        @Override
        public void run() {
            if (this.PhobosCA.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.PhobosCA.isOn() && this.PhobosCA.threadMode.getValue() == ThreadMode.WHILE) {
                    while (OyVey.eventManager.ticksOngoing()) {}
                    if (this.PhobosCA.shouldInterrupt.get()) {
                        this.PhobosCA.shouldInterrupt.set(false);
                        this.PhobosCA.syncroTimer.reset();
                        this.PhobosCA.thread.interrupt();
                        break;
                    }
                    this.PhobosCA.threadOngoing.set(true);
                    OyVey.safetyManager.doSafetyCheck();
                    this.PhobosCA.doPhobosCA();
                    this.PhobosCA.threadOngoing.set(false);
                    try {
                        Thread.sleep(this.PhobosCA.threadDelay.getValue());
                    }
            ...
            else if (this.PhobosCA.threadMode.getValue() != ThreadMode.NONE && this.PhobosCA.isOn()) {
                while (OyVey.eventManager.ticksOngoing()) {}
                this.PhobosCA.threadOngoing.set(true);
                OyVey.safetyManager.doSafetyCheck();
                this.PhobosCA.doPhobosCA();
