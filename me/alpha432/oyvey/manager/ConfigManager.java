...
public class ConfigManager implements Util
{
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("ccshack").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        if (files.contains(new File("ccshack/" + name + "/"))) {
            this.config = "ccshack/" + name + "/";
        }
        else {
            this.config = "ccshack/config/";
        }
    ...
    public boolean configExists(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("ccshack").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        return files.contains(new File("ccshack/" + name + "/"));
    }
