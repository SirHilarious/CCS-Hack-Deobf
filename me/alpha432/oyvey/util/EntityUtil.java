...
public class EntityUtil implements Util
{
    ...
    public static boolean isFakeLocalPlayer(final Entity entity) {
        return entity != null && entity.getEntityId() == -1337420 && EntityUtil.mc.player != entity;
    }
    ...
    public static boolean isInterceptedByOther(final Entity e) {
        for (final Entity entity : EntityUtil.mc.world.loadedEntityList) {
            if (entity.equals((Object)e)) {
                continue;
            }
            if (entity instanceof EntityItem) {
                continue;
            }
            return EntityUtil.mc.world.getEntitiesWithinAABB((Class)Entity.class, e.getEntityBoundingBox()).isEmpty();
        }
        return false;
    }
    ...
    public static Vec3d[] getUnsafeBlockArray(final Entity entity, final double height, final boolean floor) {
    ...
    public static List<Vec3d> getUntrappedBlocks(final EntityPlayer player, final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop) {
        final ArrayList<Vec3d> vec3ds = new ArrayList<Vec3d>();
        if (!antiStep && getUnsafeBlocks((Entity)player, 2.0, false).size() == 4) {
            vec3ds.addAll(getUnsafeBlocks((Entity)player, 2.0, false));
        }
    ...
    public static List<Vec3d> targets(final Vec3d vec3d, final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop, final boolean raytrace) {
        ...
        if (antiStep) {
            ...
            final List<Vec3d> vec3ds = getUnsafeBlocksFromVec3d(vec3d, 2.0, false);
            if (vec3ds.size() == 4) {
    ...
    public static List<Vec3d> getTrapOffsetsList(final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop) {
        final ArrayList<Vec3d> offsets = new ArrayList<Vec3d>(getOffsetList(1.0, false));
        offsets.add(new Vec3d(0.0, 2.0, 0.0));
        if (antiScaffold) {
            offsets.add(new Vec3d(0.0, 3.0, 0.0));
        }
        if (antiStep) {
            offsets.addAll(getOffsetList(2.0, false));
        }
        if (legs) {
            offsets.addAll(getOffsetList(0.0, false));
        }
        if (platform) {
            offsets.addAll(getOffsetList(-1.0, false));
